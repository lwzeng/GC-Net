...
parser.add_argument('--hierarchy', default=True, type=bool, help='worker number')
...
if parser.parse_args().hierarchy == True:
    # saveMode = Save_path+'/classifier/vgg16/hierarchy/'
    saveMode = "weights/vgg16/hierarchy"
else:
    saveMode = Save_path+'/classifier/vgg16/single/'
parser.add_argument('--save_folder', default=saveMode, help='Directory for saving checkpoint models')
parser.add_argument('--log_folder', default='logs/vgg16', help='Directory for saving log of tensorboard')
...
if args.hierarchy == True:
    ...
    NetMode = models.vgg16(pretrained=True)
NetMode.classifier[6] = tnn.Linear(in_features=4096, out_features=200, bias=True)
NetMode.cuda()
...
for name, value in NetMode.named_parameters():
    # print(name)
    if 'classifier.6' in name or "classifier_r" in name or "classifier_f" in name:
        if 'weight' in name:
